...
class L10nEsVatBookIgic(models.Model):
    ...
    line_ids = fields.One2many(
        ...
        string=_("Issued/Received invoices"),
        copy=False,
        readonly="True",
    )
    ...
    rectification_issued_line_ids = fields.One2many(
        ...
        string=_("Issued Refund Invoices"),
        copy=False,
        readonly="True",
    )
    ...
    rectification_received_line_ids = fields.One2many(
        ...
        string=_("Received Refund Invoices"),
        copy=False,
        readonly="True",
    )
    ...
    tax_summary_ids = fields.One2many(
        comodel_name="l10n.es.vat.book.igic.tax.summary",
        string=_("Tax Summary"),
        inverse_name="vat_book_id",
        readonly="True",
    )
    ...
    summary_ids = fields.One2many(
        comodel_name="l10n.es.vat.book.igic.summary",
        string=_("Summary"),
        inverse_name="vat_book_id",
        readonly="True",
    )
    ...
    auto_renumber = fields.Boolean(
        _("Auto renumber invoices received"), states={"draft": [("readonly", False)]}
    )
    ...
    def _compute_error_count(self):
        vat_book_exception_group = self.env["l10n.es.vat.book.igic.line"].read_group(
    ...
    def _create_vat_book_tax_summary(self, tax_summary_data_recs):
        return self.env["l10n.es.vat.book.igic.tax.summary"].create(
    ...
    def _create_vat_book_summary(self, tax_summary_recs, book_type):
        return self.env["l10n.es.vat.book.igic.summary"].create(
    ...
    def get_special_taxes_dic(self):
        map_lines = self.env["atc.vat.book.igic.map.line"].search(
            [("special_tax_group", "!=", False)]
        )
        special_dic = {}
    ...
    def create_vat_book_lines(self, move_lines, line_type, taxes):
        VatBookLine = self.env["l10n.es.vat.book.igic.line"]
        moves_dic = {}
    ...
    def _calculate_vat_book(self):
        ...
        for rec in self:
            ...
            if rec.auto_renumber:
                rec_invs = self.env["l10n.es.vat.book.igic.line"].search(
    ...
    def view_issued_invoices(self):
        self.ensure_one()
        report_name = "l10n_es_vat_book_igic.report_vat_book_igic_invoices_issued_html"
        return (
    ...
    def view_received_invoices(self):
        self.ensure_one()
        report_name = "l10n_es_vat_book_igic.report_vat_book_igic_invoices_received_html"
        return (
            self.env["ir.actions.report"]
            .search(
                [("report_name", "=", report_name), ("report_type", "=", "qweb-html")],
    ...
    def export_xlsx(self):
        self.ensure_one()
        return self.env.ref("l10n_es_vat_book_igic.l10n_es_vat_book_igic_xlsx").report_action(
            self
        )
